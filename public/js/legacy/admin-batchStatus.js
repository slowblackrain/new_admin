function check_deliveryCompany(shipping_seq)
{
	...
	if(obj_option.val()==gf_deliveryCode){
...
function go_search_export()
{
	...
	if( shipping_method == 'delivery' || shipping_method == '' ) {
	...
	$.ajax({
		...
		success: function(result){
			if	(result){
				...
				openDialog("일괄출고처리", "batch_status_popup_layer", {"width":"930","height":"250","close":function(){document.location.reload();}});
			}
		}
	});
}
...
function check_all(obj)
{
	...
	$("input[class='check_export_code']").each(function(){
...
function invoice_link(export_code){
	...
	if( url && number ){
		if(company=='code12'){
			var number1 = number.substr(0,4);
			var number2 = number.substr(4,3);
			var number3 = number.substr(7,number.length);
			url = url+number1+'&billno2='+number2+'&billno3='+number3;
		}else{
			url += number;
		}
		window.open(url);
