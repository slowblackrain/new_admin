				...
				if	(that.mobile_mode){
					...
					if($('input.suboption') != ""){
						$('input.suboption').each(function(  ) {
							...
							if(title.indexOf('인쇄') != -1){
								alert("인쇄는 1가지 타입만 가능합니다.");
								cancel = 'y';
								return false;
							}
					...
					if(!cancel){
						result += '<tr class="quanity_row suboption_tr" opt_group="' + apply_option_seq + '">';
						result += '<td class="option_text quantity_cell_sub"><span id = "sop_n[]" class="btn_small_normal">추가</span> ' + msg + '</td>';
						result += '<td class="quantity_cell_sub" align="center">';
						result += '	<table border="0" cellpadding="1" cellspacing="0">';
						result += '	<tr>';
						if(title.indexOf('인쇄') != -1 || title.indexOf('샘플구매') != -1 ){
							result += '		<td><input type="hidden" id ="' + that.input_suboption_ea_name + '"  name="' + that.input_suboption_ea_name + '[' + apply_option_seq + '][' + idx + ']" value="1" class="onlynumber ' + that.ea_input_class + '" /></td>';
						}else{
	...
	this.calculate_goods_price	= function(){
		...
		$('.'+this.result_table_class+' .' + this.suboption_price_class).each(function(i){
			...
			*/
			...
			if(suboption == "볼펜"){
				if(tot_ea < 1000) film_price = 20000;
				if(tot_ea < 1000) {
					price = 80;
				} else if(tot_ea >= 1000 && tot_ea < 3000) {
					price = 40;
				} else if(tot_ea >= 3000 && tot_ea < 100000) {
					price = 20;
				} else {
					price = 80;
				}
			}else if(suboption == "국내1도(200)"){
				if(tot_ea < 500) film_price = 20000;
			}else if(suboption == "국내1도(150)"){
				if(tot_ea < 1000) film_price = 20000;
			}else if(suboption == "국내볼펜(150)"){
				if(tot_ea < 1000) film_price = 20000;
			}else if(suboption.indexOf('인쇄1도') != -1 ){
				if(tot_ea < 500) film_price = 20000;
			}else if(suboption.indexOf('자수(300)') != -1 || suboption.indexOf('자수(500)') != -1 ){
				if(tot_ea < 200) {
					openDialogAlert('200개 미만 주문시고객센터 (1566-6779)로<br>별도 연락 부탁드립니다.', 400, 180, function(){
						location.reload();
					});
					return false;
				}
