$(function(){
	...
	if(dashBoardObj[0]){
		...
		var defaultDashBoardTop = parseInt($(dashBoardObj.selector).offset().top);
		$("#wrap").bind('scroll resize',function(){
			//console.log(defaultDashBoardTop);
			var scrollTop = parseInt($("#wrap").scrollTop());
	...
	/* 상단 대쉬보드 스크롤처리 */
	/*
	var dashBoardObj2 = $("#page-title-bar2");
	if(dashBoardObj2[0]){
		var defaultDashBoardTop2 = parseInt($(dashBoardObj2.selector).offset().top);
		$("#wrap").bind('scroll resize',function(){
			var scrollTop2 = parseInt($("#wrap").scrollTop());

			if(scrollTop2>defaultDashBoardTop2)
			{	
				dashBoardObj2.addClass('display_view');
				dashBoardObj2.addClass('flyingMode2');
				dashBoardObj2.find("span.btn.black").removeClass('black').addClass('cyanblue');
				dashBoardObj2.find(".icon-arrow-right").removeClass('icon-arrow-right').addClass('icon-arrow-right_gray');
			}
			else
			{
				dashBoardObj2.removeClass('flyingMode2');
				dashBoardObj2.find("span.btn.cyanblue").removeClass('cyanblue').addClass('black');
				dashBoardObj2.find(".icon-arrow-right_gray").removeClass('icon-arrow-right_gray').addClass('icon-arrow-right');
				dashBoardObj2.addClass('display_hidden');
			}
		});
	}
	*/
	...
	var dashBoardObj2 = $("#page-title-bar2");
	if(dashBoardObj2[0]){
		var defaultDashBoardTop2 = parseInt(dashBoardObj2.offset().top);
		$(window).bind('scroll resize',function(){
			var scrollTop2 = parseInt($(document).scrollTop());
	...
	$("div.gabia-pannel").each(function(){
		...
		$.ajax({
			...
			'data' : {'code':code,'getdata':getdata},
			'global' : false,
...
function divExcelDownload(title, selector){
	...
	if(selector == '#account_table'){
		clone.find("a").each(function(){
			...
			var dmin = lnk[0] + '//' + lnk[1].split('/')[0];
			$(this).attr("href", dmin + '/admin/account/' + $(this).attr("href"));
		});
	}
...
function printWarehouseView(offersno){
	window.open('/admin/scm_doto_warehousing/sorder_print?offersno='+offersno, '', 'width=850px,height=800px,toolbar=no,location=no,resizable=yes,scrollbars=yes');
}
...
//연락처복사
function phone_copy(phone1,phone2,phone3){
	var tmpTextarea = document.createElement('textarea');
	tmpTextarea.value =  phone1 + "-" + phone2 + "-" + phone3;
...
//상품명복사
function goods_name_copy(scode,code,sortcd,name){
	var tmpTextarea = document.createElement('textarea');
	if(!scode) scode = "";
	if(!code) code = "";
	if(!sortcd) sortcd = "";
	if(!name) name = "";
    tmpTextarea.value = "["+scode+"]"+" ("+code+")"+" ["+sortcd+"] "+name;
...
//상품명복사
function goods_name_orderview_copy(scode,code,sortcd,name){
	var tmpTextarea = document.createElement('textarea');
	if(!scode) scode = "";
	if(!code) code = "";
	if(!sortcd) sortcd = "";
	if(!name) name = "";
    var txt = "["+scode+"]"+" ("+code+")"+" ["+sortcd+"] "+name;
	tmpTextarea.value = txt;
	...
	var memo_txt = $("#cs_memo_txt").val();
	var insert_memo = "";
	
	if(memo_txt){
		insert_memo= memo_txt + "\r\n" + txt;
		$("#cs_memo_txt").val(insert_memo);
	}else{
		$("#cs_memo_txt").val(txt);
	}		
}
...
//바코드복사
function goods_barcode(barcode){
	var tmpTextarea = document.createElement('textarea');
	if(!barcode) barcode = "";
    var txt = barcode;
	tmpTextarea.value = txt;
...
//사진명복사
function goods_img_copy(goods_seq){
	if(goods_seq){
		$.ajax({
			url: "/admin/goods/goods_img_copy",
			type: "get",
