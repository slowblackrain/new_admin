...
function go_search_export(bundle_mode)
{
	...
	var able_select_consignment = false;
	...
	$("input[name='base_inclusion[]']:checked").each(function(){
			url += "&base_inclusion[]="+$(this).val();
			if($(this).val() == 2){
				able_select = true;
			}
			if($(this).val() == 1){
				able_select_consignment = true;
			}
	});

	if(provider_seq && able_select){
		url += "&provider_seq="+provider_seq;
	}

	if(provider_seq_consignment && able_select_consignment){
		url += "&provider_seq_consignment="+provider_seq_consignment;
	}

	var shipping_method = $("select[name='search_shipping_method'] option:selected").val();
	if( shipping_method ){
		url += "&search_shipping_method="+shipping_method;
	}
...
function package_check_barcode(shipping_group_seq,upperstr,barcode_obj){
	var str_check_ea = 'package_barcode_ea['+shipping_group_seq+']['+upperstr+']';
	var check_ea_objs = $("input[name='"+package_str_check_ea+"']");
	check_ea_objs.each(function(){
		var check_ea_obj = $(this);
		var export_ea_obj = check_ea_obj.closest("td").prev().find("select.export_ea");
		...
		if( export_ea == 0){
			...
			// 체크완료
			if( check_complete(shipping_group_seq) ){
...
function check_barcode(obj,mode){
...
function export_each_popup(each_shipping_seq,each_item_option_seq,each_shipping_method)
{
	...
	if(sel_shipping.val() == gf_deliveryCode){
		goodsflow_set(true);
	}else{
		goodsflow_set(false);
	}
...
function export_bundle_delivery(mode)
{
	...
	$('.' + check_list).each(function(){
		if	(!$(this).attr('linkage_order_id')){
			...
			linkage_order_id.push($(this).attr('linkage_order_id'));
		}
	});
	...
	if(mode != 'provider_bundle'){
		...
		if(order_seq_list.length < 2){
			if(linkage_order_id.length > 0) {
				alert("외부연동 주문건의 경우 합포장 출고를 진행할 수 없습니다.\n개별 출고처리를 진행해주시기 바랍니다.");
			} else {
...
function make_bundle_list(){
	...
	$.get('../order/order_export_popup', params, function(response){
		...
		openDialog("합포장 출고 처리", "to_be_bundle", {"width":"1200","height":"450","close" : close_bundle_lay});
	...
	$.ajax({
		...
		success: function(result){
			if	(result){
				...
				openDialog("일괄출고처리", "batch_status_popup_layer", {"width":"930","height":"250","close":close_callback});
			}
		}
	});
}
...
function check_all(obj)
{
	...
	$("input[class='check_shipping_group_seq']").each(function(){
...
{
	var obj = $('table.search-form-table');
