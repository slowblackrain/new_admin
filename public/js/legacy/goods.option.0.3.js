...
var jscls_option_select	= function(){
	...
	this.set_init		= function(n){
		...
		if	(arguments[2])	this.org_goods_price				= arguments[2];
		...
		if	(arguments[15])	this.vat_flag						= arguments[15];
	...
	this.calculate_goods_price	= function(){
		...
		$('.'+this.result_table_class+' .' + this.suboption_price_class).each(function(i){
			...
			*/
			...
			if(that.vat_flag == 1) 
			{
				...
				film_price = Math.round(film_price * 1.1/10) * 10;
			}

			if(v_show == 1)
			{
				if(v_make == 1) $('.' + that.input_suboption_ea_name+'_film').eq(i).html("<br>제작비용 : "+that.comma(film_price)+"원");
				else $('.' + that.input_suboption_ea_name+'_film').eq(i).html("<br>패드비용 : "+that.comma(film_price)+"원(500개당 1개무료)");
		...
		$('.first_label').text(ea);
		$('.first_price').text(price);
		$('.first_totprice').text(this.comma(price*ea));

		if(that.vat_flag == 0)
		{
			v_dis = this.comma(this.org_goods_price) + ' X ' + this.comma(ea) + ' = ';
			v_per = 100 - Math.round(goods_price / this.org_goods_price * 100);

			if(this.multi_discount_ea_fifty <= ea && this.multi_discount_ea_hundred > ea && this.multi_discount_ea_fifty > 0){
				//도매할인가 수량에 맞을때
				v_dis = v_dis + '<strike>' + this.comma(this.org_goods_price*ea) + '</strike>원 ' + this.comma(goods_price*ea) + '원(도매할인가 '+v_per+'% 할인적용)';
			}else if(this.multi_discount_ea_hundred <= ea && this.multi_discount_ea_hundred > 0){
				//수입특가 수량에 맞을때
				v_dis = v_dis + '<strike>' + this.comma(this.org_goods_price*ea) + '</strike>원 ' + this.comma(goods_price*ea) + '원(수입가 '+v_per+'% 할인적용)';
			}else{
				v_dis = v_dis + this.comma(this.org_goods_price*ea);
			}
	...
	this.calculate_muti_discount	= function(ea, price){
		...
		if(this.vat_flag == 0)
		{
			if(this.multi_discount_ea_fifty <= ea && this.multi_discount_ea_hundred > ea && this.multi_discount_ea_fifty > 0){
				...
				this.goods_price = Math.round(this.org_goods_price * 1.1/10) * 10;
				return Math.round(this.org_goods_price * 1.1/10) * 10;
		}	
	};
