...
@section('content')
<div class="find_id_wrap" style="width: 400px; margin: 50px auto; padding: 20px; border: 1px solid #ddd;">
    <h2 style="text-align: center; margin-bottom: 20px; font-size: 20px; font-weight: bold;">아이디 찾기</h2>
    
    <form action="{{ route('member.find_id_result') }}" method="POST">
        @csrf
        ...
        <div class="btn_group">
            <button type="submit" style="width: 100%; height: 40px; background: #333; color: #fff; border: none; font-size: 14px; cursor: pointer;">아이디 찾기</button>
        </div>
        
        <div class="links" style="margin-top: 10px; text-align: center; font-size: 12px;">
            <a href="{{ route('member.find_pw') }}" style="color: #666; text-decoration: underline;">비밀번호 찾기</a> | 
